...
XYZ trackerXYZ[NUM_TRACKERS];       // corrected tracker positions
XYZ baseMeasuredXYZ[NUM_TRACKERS];  // base's ToF measurements
...
void measureTrackers() {
  if(newMeasurement){
    ...
    // update corrected XYZ per tracker
    for(int i=0;i<NUM_TRACKERS;i++){
      if(trackerXYZ[i].x>0){
        ...
        // two-way correction: average tracker measurement with base measurement
        trackerXYZ[i].x=(trackerXYZ[i].x + baseMeasuredXYZ[i].x)/2.0;
        trackerXYZ[i].y=trackerXYZ[i].y; // no change
        trackerXYZ[i].z=trackerXYZ[i].z;
      }
    }
...
void receiveTrackerPackets() {
  ...
  if(size>0){
    ...
    for(int i=0;i<NUM_TRACKERS;i++){
      if(trackerReceived[i].x>0) trackerXYZ[i]=trackerReceived[i];
    }
  }
}
